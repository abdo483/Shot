...
module.exports = {
    name: "spank",
    category: "pet play",
    description: "Spanks the mentioned user. User must be your pet or have the your server's **Shared Role**.",
    usage: `${(process.env.PREFIX)}spank <@user>`,
    run: async (client, message, args) => {
        ...
        const neko = new Neko();
        const url = await neko.nsfw.spank();
        ...
        if(!pet){
            if(!member.roles.cache.has(sharedRole.id)) {
                ...
                const embed = new MessageEmbed()
                    ...
                    .setDescription(`${message.author} **spanked** ${member}`)
                    .setImage(url.url);
